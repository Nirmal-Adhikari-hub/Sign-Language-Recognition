...
def get_args():
    ...
    parser.add_argument('--drop_rate', default=.1, type=float)
    parser.add_argument('--drop_path_rate', default=.1, type=float)
    parser.add_argument('--head_drop_rate', default=.1, type=float)
    ...
    parser.add_argument('--weight_decay', default=.0001, type=float)
    ...
    parser.add_argument('--n_frames', default=256, type=int)
...
def main(args, ds_init):
    ...
    # Train dataset
    train_dataset = build_dataset(modal=args.modal, gloss_tokenizer=gloss_tokenizer, is_train=True, is_test=False, args=args)
    ...
    test_dataset = build_dataset(modal=args.modal, gloss_tokenizer=gloss_tokenizer, is_train=False, is_test=True, args=args)
    ...
    if args.modal == 'video':
        assert args.depth == 24 and args.embed_dim == 192

        model = model_class(
            pretrained=args.pretrained,
            width=args.video_size[1], height=args.video_size[0], 
            patch_size=args.patch_size,
            channels=args.in_chans,
            drop_rate=args.drop_rate, drop_path_rate=args.drop_path_rate,
            fused_add_norm=True, rms_norm=True, residual_in_fp32=True,
            bimamba=True,
            ...
            head_drop_rate=args.head_drop_rate, n_classes=args.n_classes
        )
    ...
    args.lr = args.lr * total_batch_size / 256 # 업데이트 시 배치 크기와 데이터 증강을 고려한 lr 조정
    args.min_lr = args.min_lr * total_batch_size / 256 # 업데이트 시 배치 크기와 데이터 증강을 고려한 lr 조정
    args.warmup_lr = args.warmup_lr * total_batch_size / 256 # 업데이트 시 배치 크기와 데이터 증강을 고려한 lr 조정
    ...
    print(f"Max WD: {max(wd_schedule_values):.6f}, Min WD: {min(wd_schedule_values):.6f}, Warmup WD: {args.weight_decay_start:.6f}")
