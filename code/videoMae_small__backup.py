...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        # print(f"======================================X shape in forward_features: {x.shape}")
        ...
        if self.pos_embed is not None:  
            # print(f"===========A Postional Embedding Shape: {self.pos_embed.shape}")
            pos_embed = self.pos_embed.reshape(t, -1, width)
            # print(f"===========B Postional Embedding Shape: {pos_embed.shape}")
            # num_extra_tokens = 
            pos_embed = interpolate_pos_embed_online(
                pos_embed, self.grid_size, [h, w], 0).reshape(1, -1, width)
            # print(f"===========C Postional Embedding Shape: {pos_embed.shape}")
        ...
        # x = x.mean(dim=[2,3,4], keepdim=False)  # [b num_classes] -> Global Pooling
        # rois = self.pooler(x, proposals)  # [n c 7 7]
        # rois = self.max_pooler(rois).view(rois.size(0), -1)  # [n c]
        # return rois
        print(f"======================================X returned from forward_features: {x.shape}")
        return x
    ...
    def forward(self, x):
        ...
        print(f"Encoder Output: {x.mean(dim=2, keepdims=False).detach().cpu().numpy()}")
        ...
        # print(f"Shape after head: {self.head(x.flatten(2).mean(dim=-1)).shape}")
        x = self.head(x)  # Global average pooling
        # print(f"Shape after head: {x.shape}")
        print(f"Head weights: {self.head.weight.detach().cpu().numpy()}")
        return x
...
def interpolate_pos_embed_online(
    ...
    # print(f"Positional embeddings shape before interpolation: {pos_embed.shape, pos_embed[:1]}")
    # print(f"Original size: {orig_size}, New size: {new_size}")
    # print(f"Extra tokens: {num_extra_tokens}, Patch tokens shape: {pos_tokens.shape}")
