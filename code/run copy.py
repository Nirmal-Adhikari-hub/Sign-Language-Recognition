...
def get_args():
    parser = argparse.ArgumentParser('Single modal training and evaluation script for Continuous SLR')
    ...
    parser.add_argument('--multimodal',
                        action='store_true',
                        help='Flag for multimodal training state indication')
    ...
    parser.add_argument('--trimmed', default=180, type=int)
    parser.add_argument('--time_stride', default=60, type=int)
    parser.add_argument('--max_len', default=2048, type=int)
    parser.add_argument('--data_set', default='Condensed_movies', type=str)
...
def main(args, ds_init):
    ...
    # Train dataset
    train_dataset, all_genres, cls_cnt_list = build_dataset(modal=args.modal, is_train=True, is_test=False, args=args)

    # 학습 시 validation data를 통한 평가 여부
    if args.disable_eval_during_training:
        val_dataset = None
    else:
        val_dataset, _, _ = build_dataset(modal=args.modal, is_train=False, is_test=False, args=args)
    test_dataset, _, _ = build_dataset(modal=args.modal, is_train=False, is_test=True, args=args)
    ...
    collate_fn = partial(multiple_samples_collate, fold=False) if args.n_samples > 1 else None
