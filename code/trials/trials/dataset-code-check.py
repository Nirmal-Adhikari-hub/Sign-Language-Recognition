...
class Phoenix2014Video(Dataset):
    def __init__(
        ...
        with gzip.open(anno_path) as f:
            self.annotations = pickle.load(f)[:10]
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            return frames, n_frames, gloss_ids, gloss_len, video_name
    ...
    def save_debug_images(self, frames, prefix="debug_frame"):
        frames = frames[0, :, :, :, :]
        for i, frame in enumerate(frames.transpose(0, 1)):
            denormalized_frames = tensor_denormalize(frame, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
            pil_frame = transforms.ToPILImage()(denormalized_frames)
    ...
    if args.dataset == 'phoenix-2014':
        ...
        if modal == 'video':
            ...
            dataset = Phoenix2014Video(
                ...
                clip_len=args.n_frames, aug_size=args.aug_size, target_size=args.video_size, args=args, gloss_tokenizer=gloss_tokenizer
            )
        elif modal == 'keypoint':
            if is_train == True:
                mode = 'train'
