...
def get_selected_indexs(vlen, tmin=1, tmax=1, max_num_frames=400):
    ...
    # tmin, tmax가 1이 아닌 경우: 무작위 temporal augment 로직 -----------------------
    min_len = int(tmin * vlen)
    max_len = min(max_num_frames, int(tmax * vlen))

    # min_len ~ max_len 범위에서 임의의 길이 chosen_len 선택
    ...
    if args.dataset == 'phoenix-2014':
        ...
        if modal == 'video':
            ...
            dataset = Phoenix2014Video(
                ...
                clip_len=args.n_frames, aug_size=args.aug_size, target_size=args.video_size, gloss_tokenizer=gloss_tokenizer, args=args
            )
        elif modal == 'keypoint':
            if is_train == True:
                mode = 'train'
