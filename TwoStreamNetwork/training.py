...
if __name__ == "__main__":
    ...
    debug_counter = [0]
    # Register hooks for debugging layers
    print(f"****************************************************************************")
    def hook_fn(module, input, output):
        if debug_counter[0] < max_debug_iterations:
            logger.info(f"Layer: {module.__class__.__name__}")
            logger.info(f"Input Shape: {[i.shape for i in input]}")
            logger.info(f"Output Shape: {output.shape if isinstance(output, torch.Tensor) else type(output)}")
            debug_counter[0] += 1

    # Check if the model is already wrapped in DDP
    model_to_hook = model.module if hasattr(model, "module") else model

    # Register hooks
    for name, layer in model_to_hook.named_modules():
    ...
    for epoch_no in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            # ============================================================================
            if step < max_debug_iterations:  # Limit debug prints
                print(f"****************************************************************************")
                logger.info(f"Batch[{step}] Keys: {list(batch.keys())}")
                for key, value in batch.items():
                    if isinstance(value, torch.Tensor):
                        logger.info(f"Batch[{key}] Shape: {value.shape} | Dtype: {value.dtype}")
                    else:
                        logger.info(f"Batch[{key}]: {type(value)}")
                print(f"****************************************************************************")
            #  ============================================================================
