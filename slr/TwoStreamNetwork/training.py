...
if __name__ == "__main__":
    ...
    for epoch_no in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            print(batch['recognition_inputs']['gloss_labels'])
            """
            Input

            - name:           ['fullFrame-210x260px/train/18November_2009_Wednesday_tagesschau_default-8/1/18November_2009_Wednesday_tagesschau', 'fullFrame-210x260px/train/16July_2011_Saturday_tagesschau_default-7/1/16July_2011_Saturday_tagesschau']
            - gloss:          ['KUESTE MORGEN AUCH STURM loc-NORD SUED MAESSIG MAESSIG WEHEN WEHEN', 'ANFANG FREUNDLICH DANN IM-VERLAUF AUCH REGEN REGEN DANN MEISTENS GEWITTER GEWITTER NORDWESTRAUM WIEDER KLAR __OFF__']
            - text:           ['', '']
            - num_frames:     [138, 160], 변동 가능
            - gls_lengths:    torch.Size([2])
            - gloss_labels:   torch.Size([2, 15]) # 변동 가능
            - sgn_videos:     torch.Size([2, 148, 3, 260, 210]) # 변동 가능
            - sgn_keypoints:  None
            - sgn_lengths:    torch.Size([2])

            Output

            gloss_feature:              torch.Size([2, 37, 512])
            gloss_feature_norm:         torch.Size([2, 37, 512])
            gloss_logits:               torch.Size([2, 37, 1235])
            gloss_probabilities_log:    torch.Size([2, 37, 1235])
            gloss_probabilities:        torch.Size([2, 37, 1235])
            valid_len_out:              torch.Size([2])
            head_rgb_input:             torch.Size([2, 37, 832])
            valid_len_out_lst:          torch.tensor([48, 51]), 변동 가능
            input_lengths:              torch.tensor([29, 47]), 변동 가능
            recognition_loss:           torch.tensor(229.7334), 변동 가능
            total_loss:                 torch.tensor(229.7334), 변동 가능
            gloss_feature_ssl:          None
